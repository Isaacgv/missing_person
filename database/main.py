...
def get_usr_submission():
    query = f"select id, face_encoding from user_submissions where status='NR'"
    with PostgresConnection() as conn:
        cursor = conn.cursor()
        cursor.execute(query)
        return cursor.fetchall()
...
@app.get('/change_found_status_send')
def change_found_status_send(id: str):
    query = f"update user_submissions set status='R' where id={id}"
    with PostgresConnection() as conn:
        cursor = conn.cursor()
        cursor.execute(query)
    return {"status": "success"}
